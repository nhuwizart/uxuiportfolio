...
function getShipping(qty, obj, isMain){
	switch(qty){
		case 2:
			if(obj.qty === 1){
				obj.product.shippings[0].shippingMethodId = "10005";
			} else if(obj.qty === 2){
				obj.product.shippings[0].shippingMethodId = "10006";
			}
		...
		case 3:
			if(obj.qty === 1){
				...
				obj.product.shippings[0].shippingMethodId = "10007";
			}
		...
		case 4:
			if(obj.qty === 1){
				...
				obj.product.shippings[0].shippingMethodId = "10008";
			}
...
function updateProductPriceWithOption(){
	...
	Array.from(elmLists).forEach(item => {
		...
		Array.from(elmSets).forEach(set => {// split option with size, color and qty
			...
			const color = elmColor.value || "Black";
